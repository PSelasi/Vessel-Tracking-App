...
class AisSentence {
    ...
    constructor(message) {
        ...
        if (startIndex === -1) {
            throw new errors_1.DecodingError('Start not found', this.message);
        }
        const messageFields = this.message.split(',');
        if (messageFields.length !== 7) {
            throw new errors_1.DecodingError('Invalid length', this.message);
        }
        const suffix = messageFields[6].split('*');
        if (suffix.length !== 2) {
            throw new errors_1.DecodingError('Invalid suffix', this.message);
        }
    ...
    checkChecksum() {
        ...
        if (checksumHex !== this.checksum) {
            throw new errors_1.DecodingError('Invalid checksum', this.message);
        }
    }
}
