/// <reference types="node" />
import { Transform, TransformOptions } from 'stream';
import AisSentence from './ais-sentence';
interface AisDecoderOptions {
    silent?: boolean;
}
declare class AisDecoder extends Transform {
    options: AisDecoderOptions;
    multiPartBuffer: Array<AisSentence>;
    constructor(options?: AisDecoderOptions, transformOptions?: TransformOptions);
    _transform(chunk: Buffer, encoding: string, callback: (error?: Error) => void): void;
    handleMultiPartSentence(sentence: AisSentence): void;
    decodePayload(payload: string, channel: string, sentences: Array<AisSentence>): void;
}
export default AisDecoder;
